...
func (parser *Parser) findPkgDir(typeName string, file *ast.File) (string, error) {
	typeSpecDef := parser.packages.FindTypeSpec(typeName, file)
	if typeSpecDef == nil {
		return "", fmt.Errorf("cannot find type definition: %s", typeName)
	}

	fileInfo, ok := parser.packages.files[typeSpecDef.File]
	if !ok {
		return "", fmt.Errorf("findQuerySourcePath, i dont know why now")
	}

	return filepath.Dir(fileInfo.Path), nil
}
...
func (parser *Parser) getTypeSchema(typeName string, file *ast.File, ref bool) (*spec.Schema, error) {
	...
	fmt.Println("getTypeSchema", typeName, typeSpecDef)
